...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  return <div className="admin-page"><aside className={`admin-sidebar${open ? " open" : ""}`}><div className="admin-brand"><Brand /></div><p className="admin-sidebar__label">Khu quản trị</p><nav className="admin-nav">{links.map(([href,label]) => <Link key={href} className={pathname === href ? "active" : ""} href={href} onClick={() => setOpen(false)}>{label}</Link>)}<Link href="/">Xem cửa hàng</Link></nav><button className="admin-logout" type="button" onClick={async () => { await logout(); router.push("/login"); }}>Đăng xuất</button></aside><main className="admin-main"><header className="admin-topbar"><button className="admin-menu-toggle" type="button" aria-label="Mở menu" onClick={() => setOpen((value) => !value)}><span /><span /><span /></button><div><p>Xin chào,</p><strong>{user.name}</strong></div><span className="admin-avatar">QT</span></header>{children}</main></div>;
}
